...
def FindFIO():
    ...
    try:
        ...
        #print("KK_fio: ret= ", ret)
        #print("KK_fio: out= ", out)
        #print("KK_fio: err= ", err)
        if ret == 0:
            return "fio"
    except:
        try:
...
def ParseArgs():
    ...
    hit = ""
    with open("/proc/mounts", "r") as f:        #KK_1
        mounts = f.readlines()
    #print("=============================================================")
    #print(mounts)
    #print("=============================================================")
    for l in mounts:
        ...
        if pdispart:
            ...
            if re.match('^/dev/sdp.*$', dev):
                ...
                #print("KK_000006", mnt, dev, chkdev)
        if chkdev == physDrive:
            #print("KK_000007")
            hit = dev + " on " + mnt
    if hit != "":
...
def grep(inlist, regex):
    ...
    #print("KK_grep-regex: ", regex)
    out = []
    for i in inlist:
        if re.search(regex, i):
            out = out + [i]
    #print("KK_grep-out: ", out)
    return out
...
def CollectSystemInfo():
    ...
    if 'ppc64' in uname:
        ...
        #print("KK_ cpuFreqMHz: ", cpuFreqMHz)

def VerifyContinue():
    print("-" * 50 + "VerifyContinue pass" + "-" * 50 + "\n")   #KK_ added
    return 0            #KK_ added
...
def CollectDriveInfo():
    ...
    #print("KK_pd=", physDrive, pd)
    try:
        if isFile:
            physDriveBase = os.path.basename(pd)
            physDriveBytes = str(os.stat(pd).st_size) + "\n"
            #print("KK_physDriveBytes if isFile=", physDriveBytes)
        else:
            physDriveBase = os.path.basename(pd)
            code, physDriveBytes, err = Run(['blockdev', '--getsize64', pd])
            #print("KK_physDriveBytes else isFile=", physDriveBytes)
            if code != 0:
    ...
    serial = "UNKNOWN"
    try:
        #print("KK_ try for nvme list --output-format")
        nvmeclicmd = ['nvme', 'list', '--output-format=json']
        code, nvmecli, err = Run(nvmeclicmd)
        if code == 0:
            #print("KK_nvmecli ============", nvmecli) #KK_
            j = json.loads(nvmecli)
            #print("KK_j ==================", j)
            for drive in j['Devices']:
                if drive['DevicePath'] == pd:       #KK_ pd = /dev/nvme0n1
                    model = drive['ModelNumber']    #KK_ Model Number = Samsung SSD 960 EVO 250GB
                    serial = drive['SerialNumber']  #KK_ SerialNumber = S3ESNX0HC03566B
                    return
    except:
        pass  # An error in nvme is not a problem
    try:
        #print("KK_ try for sdparm --page sn --inquiry --long")
...
def SetupFiles():
    ...
    if not outputDest:
        #print("KK_ outputDest1111111111 = ", outputDest)
        outputDest = os.getcwd()
    #print("KK_ outputDest222222222 = ", outputDest)
...
def SequentialConditioning():
    ...
    def GenerateJobfile(drive, testcapacity, testoffset):
        ...
        #print("KK_ GenerateJobfile - quickie", quickie)
    ...
    print("KK_ SequentialConditioning - cmdline", cmdline, jobfile.name)
    code, out, err = Run(cmdline)
    #print("KK_ jobfile ============================= jobfile.name is ", jobfile.name)
    #code, out, err = Run(['cat', jobfile.name]) #KK_
...
def RandomConditioning():
    ...
    if not cluster:
        ...
        for host in physDriveDict.keys():
            ...
            jobfile = jobfile + [newjob]
    cmdline = cmdline + ['--output-format=' + str(fioOutputFormat)]
    print("KK_ RandomConditioning - cmdline", cmdline, jobfile.name)
    code, out, err = Run(cmdline)
    #print("KK_ jobfile ============================= jobfile.name is ", jobfile.name)
    #code, out, err = Run(['cat', jobfile.name]) #KK_
...
def RunTest(iops_log, seqrand, wmix, bs, threads, iodepth, runtime):
    ...
    if not cluster:
        ...
        cmdline = cmdline + [jobfile.name]
        print("KK_ RunTest - if not cluster jobfile.name = ", jobfile.name, "\n")
    ...
    cmdline = cmdline + ['--output-format=' + str(fioOutputFormat)]
    print("KK_RunTest - cmdline = ", cmdline)
...
def DefineTests():
    ...
    threadslist = (1, 128)
    #'''
    shorttime = 12 #KK_ org=120  # Runtime of point tests
    longtime = 12  #KK_ org=1200  # Runtime of long-running tests
    if quickie:
    ...
    #KK_org AddTest('Sequential Preconditioning', 'Seq Pass 1', '100', '131072', '1',
    AddTest('Sequential Preconditioning', 'Seq Pass 1', '1', '131072', '1',
                    '256', False, '', 'Sequential Preconditioning Pass 1',
            lambda o: {SequentialConditioning()})

    #KK_Org if not fastPrecond:
    if fastPrecond:
        print("KK_fastPrecond is ===================== ", fastPrecond)
        #KK_org AddTest('Sequential Preconditioning', 'Seq Pass 2', '100', '131072', '1',
        AddTest('Sequential Preconditioning', 'Seq Pass 2', '10', '131072', '1',
...
def RunAllTests():
    ...
    for o in oc:
        if o['desc'] == "":
            ...
            print("\n" + "KK_ RunAllTests for header-printing job ========================")
            print("\n" + fmtstr.format("---"+o['name']+"---", "", "", ""))
            sys.stdout.flush()
            o['cmdline'](o)
        else:
            # This is a real test job, run it in a thread
            print("\n" + "KK_ RunAllTests for real test job, run it in a thread ==========")
...
quickie = True #KK_ for ezfio debuging False   # Flag to indicate short runs, only for ezfio debugging!
